...
function installNpmDummy(subDep, dep) {
    ...
    if (!fileExists(packageJson)) {
        fs.writeFileSync(packageJson, '{"name":"' + subDep.name + '","version":"' + subDep.version + '","description":"dummy"}');
        console.warn("\n  Warning:" + whatWeDo);
    } else {
...
//wrapper around runCmd, returning a deferred
function runCmdDeferred(cmd, project, opts) {
    var deferred = when.defer();
    du.runCmd(cmd, project, opts, function(stdout, stderr) {
