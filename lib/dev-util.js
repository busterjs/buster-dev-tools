...
function installNpmDummy(subDep, dep) {
    ...
    var whatWeDo =  " dummy npm install of " + subDep.kind + " " + subDep.name + " for " + this.name + " -> " + dep.name + "@" + dep.version + " -> ... -> " + subDep.name + "@" + subDep.version;
...
//    platformIs:      platformIs, // no, better use one of the below instead (or add your own here)
    ...
    onLinux:         fh.partialApply(platformIs, "linux2"), // TODO: not sure about this one (see http://nodejs.org/api/process.html#process_process_platform)
    itMatches:       itMatches,
